...
/// @source https://www.geeksforgeeks.org/minimum-enclosing-circle-set-2-welzls-algorithm/?ref=rp
...
//~ const double INF = numeric_limits<double>::max(); my redefinition, include limits
...
int main()
{
    ...
    double smallest = INF;
    Circle c = welzl(xy);
    smallest = std::min(smallest, c.R);
    c = welzl(xz);
    smallest = std::min(smallest, c.R);
    c = welzl(yz);
    smallest = std::min(smallest, c.R);
    cout << setprecision(12) << 2.0 * smallest << endl;
